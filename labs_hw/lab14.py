...
def num_splits(s, d):
    ...
    12
    """
    pss = sorted(list(powerset(s)))
    pairs = []
    for sub_x in pss:
        for sub_y in pss:
            if (
                sub_x != sub_y
                and sorted(sub_x + sub_y) == sorted(s)
                and abs(sum(sub_x) - sum(sub_y)) <= d
                and (sub_x, sub_y) not in pairs
                and (sub_y, sub_x) not in pairs
            ):
                pairs.append((sub_x, sub_y))
    return len(pairs)
