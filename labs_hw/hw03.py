...
def has_path(t, word):
    ...
    def keep_track(t, remaining_word):
        if remaining_word == "" or is_leaf(t):
            return True
        if label(t) != remaining_word[0]:
            return False
        return any([keep_track(b, remaining_word[1:]) for b in branches(t)])

    return keep_track(t, word)
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= extreme_point <= upper_bound(x):
        return interval(
            min(lower_f, upper_f, extreme_f), max(lower_f, upper_f, extreme_f)
        )
    else:
